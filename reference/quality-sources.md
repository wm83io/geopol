...
6. **Analytical-ecosystem and metric-heterogeneity independence (added 2026-06-11 via /premortem; Day 105 Mitigation 5).** Two laundering surfaces beyond anonymous-source pickup: (a) **Analytical-ecosystem co-drawing.** Named analytical outfits that routinely draw on the same intelligence ecosystem (ISW, IISS, FDD, and US-IC-briefed think tanks) are **one cluster** absent distinct sourcing; "two think tanks agree" is not two independent clusters when both are downstream of one classified briefing or each other's prior. (b) **Metric heterogeneity.** N different *metrics* (a missile-retention percentage, a storage-operability percentage, an active-site count, a commercial import-flow figure) are **not** N independent confirmations of a single proposition; each confirms a different thing. "N independent clusters confirm X" requires N sources confirming *the same proposition X* through *distinct* sourcing, not N heterogeneous measurements stacked. Where a claim is best supported by **operational demonstration** (through-use: a capability exercised in the field), cite that as the primary evidence and do not pad the confidence with metric-stacking. Canonical near-miss: the Day 105 T12 reconstitution upgrade cited "3+ independent clusters" (ISW 70% missile retention; IISS 90% underground storage; 30/33 Hormuz sites; Kpler solid-fuel imports): four heterogeneous metrics, partly one analytical ecosystem, where the real evidence was multi-axis through-use (GCC + Israel + Hormuz strikes from retained capacity). The through-use demonstration carries the upgrade; the metric stack does not multiply the cluster count.
