...
Load all of the following before writing a single word:

1. **Current synthesis** — the version being superseded (e.g. `iran-2026-sitrep-synthesis-v2-5.md`)
2. **All daily annexes since last synthesis revision** — read in chronological order. The annex chain is the authoritative record of framework drift. Appendix A is not maintained separately; the annex chain is the log.
3. **Appendix B** — current blind-spot inventory and confidence architecture
4. **Probe sweep outputs** — last 2-3 cycles minimum
5. **Latest blog-version output** if one exists — for style and structural continuity
