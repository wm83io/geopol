---
name: iran-2026-blindspot-auditor
description: >
  Audits and refreshes the Iran 2026 framework's Appendix B blind-spot inventory and standing probe
  directives. Invoke when the user says "audit the blind spots", "blind spot sweep", "refresh Appendix B",
  "what are we still missing", "where are we flying blind", or "update probe directives". Also invoke
  automatically after 2-3 probe sweeps have accumulated findings, after a framework revision in Appendix A,
  or on a monthly cadence floor (faster on trigger events). Does not run every cycle — blind spots are
  structural, not weekly. Produces an audit summary, per-BS deltas, new BSs added, and retired BSs as a
  patch against Appendix B. Use this skill whenever blind-spot auditing, structural visibility review,
  probe directive updates, or Appendix B refresh for the Iran 2026 framework is requested.
---
...
1. Latest synthesis (currently `iran-2026-sitrep-synthesis-v2-5.md`) — framework baseline
2. Current Appendix B (`iran-2026-sitrep-appendix-B-blind-spots.md`) — the document this skill modifies
3. Last 2-3 probe sweep outputs (or the most recent SITREPs containing them)
4. Current Appendix A if it has been revised since last audit

Without 2+ probe cycles to compare against, the audit has nothing to anchor to. Defer.
